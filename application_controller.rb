...
if Rails.env.production? || ENV["RESCUE_EXCEPTIONS"]
  rescue_from StanderdError, with: :rescue_internal_server_error
  rescue_from ActivRecord::RecordNotFound, with: :rescue_not_found 
...
private def  rescue_bad_request(exception)
  render "errors/bad__request", status: 400, layout: "error",
    formats: [:html]
end

private def  resucue_login_required(exception)
